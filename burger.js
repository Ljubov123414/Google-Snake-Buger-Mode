...
window.snake.burger = function(settings = {}) {
	if(settings.walls === undefined)
		settings.walls = settings.dark ? '#AAAAAA' : '#578A34';
	if(settings.border === undefined)
		settings.border = settings.dark ? '#2E2933' : settings.walls;
	...
	function processSnakeCode(code) {
		let m = code.match(
			/ s_[a-zA-Z0-9]{1,4}=function\(a\){[^}]*?=2[^]*?&&[a-zA-Z0-9_]{1,6}\(a,a\.[a-zA-Z0-9]{1,6},!0\)}}}}/
		)[0];

		let r0 = /a\.[a-zA-Z0-9]{1,6}\[0\]\.equals\(d\.[a-zA-Z0-9]{1,6}\)\|\|f\|\|g/;
		let i = m.match(r0)[0];

		let r1 = /a\.[a-zA-Z0-9]{1,8}\+\+;/;
		let s = m.match(r1)[0];
		...
		eval(
			m.replace(
				'switch',
				'if(burgerm&&burger)counters++;switch'
			).replace(
				r0,
				i + '||burgert'
			).replace(
				r1,
				`if(${s.replace('++;', '')} === 0){counter = 0;counterb=1;burger = false;}
				...
				if(counter >= 13)burger = true;`
			).replace(
				/if\(0===/,
				`if(${s.replace('++;', '')}>=a.Aa.width*a.Aa.height-6||0===`
				
			).replace(
				'WIN.play(),',
				'WIN.play(),burger=false,counter=0,counters=0,counterb=0,'
			).replace('}}}}', '}}}}')
		);

		m = code.match(
			/s_[a-zA-Z0-9_]{1,8}\.prototype\.[a-zA-Z0-9_]{1,8}=function\(a\){if\(this\.[a-zA-Z0-9_]{1,8}&&!this\.[a-zA-Z0-9_]{1,8}\)[^]*?}}/
		)[0];
		
		r0 = /[a-zA-Z0-9_]{1,8}\(this\.[a-zA-Z0-9_]{1,8}\[0<g\.type[^]*?:0\]\)/;
		i = m.match(r0)[0];
		...
		eval(
			m.replace(
				...
				r0,
				'burgerm ? burger ? burgetti : burgerImage[g.type > 0 && g.type < 13 ? g.type : 0] : ' + i
			)
		);

		m = code.match(
			/[a-zA-Z0-9_]{1,6}=function\(a\){[^}]*return c}return[^]*?}/
		)[0];

		r0 = /return a\.[a-zA-Z0-9]{1,4}/;
		let r = m.match(r0)[0];
		
		eval(
			m.replace(
				'var b=new Set;', 
				'burgerm = true;var b=new Set;'
			).replace(
		...
		m = code.match(
			/[a-zA-Z0-9_]{1,6}=function\(a\){[a-zA-Z0-9_]{1,6}\.[a-zA-Z0-9_]{1,6}\.play\(\);[^}]*?}/
		)[0];

		eval(
			m.replace(
				'.play();',
				'.play();counter = 0;burger = false;counterb = 0;'
			)
		);

		const imgs = document.body.getElementsByTagName('img');
		for(let img of imgs)
			if(img.src.includes('apple_13'))
				img.src = 'https://i.postimg.cc/rsNMmQz9/sprite-13.png';
